...
var routes = function () {
  ...
  //getting table details by using store procedure
  router.route("/GetAllTranstactionDetails/:INVENTORY/:LOCATOR/:LOT_NUMBER/:ITEM_NUMBER/:TAX_INVOICE_NUM/:SUPPLIER_ITEM_CODE/:SUPPLIER_NAME")
    .get(async function (req, res) {
      ...
      console.log("req.params.INVENTORY",req.params.INVENTORY,req.params.LOCATOR)
      var param = {
        where: {
      ...
      //Transaction quantity balance calculation
      let a = [];
        ...
        for (let j = 0; j < transferdata.length; j++) {
          for (let k = 0; k < statusdata.length; k++) {
            if (
              ...
              if (((statusdata[k].StatusId == 0 || statusdata[k].StatusId == 3 || statusdata[k].StatusId == 2 ||
                ((statusdata[k].StatusId == 1) && statusdata[k].isskip == false))) &&
                ...
                // console.log("ordermasterdetails[i].BALANACE_QTY ", ordermasterdetails[i].BALANACE_QTY);
              }
            }
          }
        }
  ...
  router.route("/GetAllheaderMappingMaster/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
    ...
    dataaccess.FindAll(transferordermasterdetails, param).then(
      ...
      function (err) {
        dataconn.errorlogger("TransactionService", "GetAllheaderMappingMaster", err);
        res.status(200).json
          ({
            Success: false,
            Message: "user has no access of GetAllheaderMappingMaster",
  ...
  router.route("/GetwarehouseDetails/:ORGANIZATION_NAME").get(function (req, res) {
    const M_HeaderMappingMaster = datamodel.M_HeaderMappingMaster();
    var param = {
      where: {
        LOCATOR_DESC: { [Op.ne]: "" },
  ...
  router.route("/GetAllLot/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    ...
    dataaccess.FindAll(transferordermasterdetails, param).then(
      ...
      function (err) {
        dataconn.errorlogger("TransactionService", "GetTransactionOrderDetailsBy", err);
        res.status(200).json
          ({
            Success: false,
            Message: "user has no access of GetTransactionOrderDetailsBy",
  ...
  router.route("/GetAllItemDetails/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
  ...
  router.route("/GetAllsupplierInvNum/:INVENTORY/:LOCATOR")
  ...
  router.route("/GetAllsupplierCode/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
  ...
  router.route("/GetAllsupplierName/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
  ...
  router.route("/GetAllLotDetails/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
  ...
  router.route("/GetAllItem/:INVENTORY/:LOCATOR").get(function (req, res) {
    const transferordermasterdetails = datamodel.transferordermasterdetails();
    var param = {
      where: {
        INVENTORY: req.params.INVENTORY,
  ...
  router.route("/getAllLocatonIds/:LocationId").get(function (req, res) {
    ...
    var param = {
      attributes: [
        [sequelize.fn("DISTINCT", sequelize.col("LocationName")), "LocationName"],
      ],
      order: ["LocationName"],
    };
  ...
  router.route("/GetAllTranstactionMappingDetails").get(function (req, res) {
    const M_TransferOrder = datamodel.M_TransferOrder();
    var param = {
      attributes: [
        "TransferOrder_ID",
    ...
    dataaccess.FindAll(M_TransferOrder, param).then(
      ...
      function (err) {
        dataconn.errorlogger("TransactionService", "TransferMappingDetails", err);
  ...
  /////////createTransferOrderItemsDetails
            ...
            function (result) {
              ...
              //  console.log("reqBody.orderDetails", reqBody.orderDetails);
                ...
                Promise.all(promiseDetails).then(function () {
                  dataaccess.BulkCreateWithTransaction(orderDetails, mapDetails, trans).then(
                    (resultfinal) => {
  ...
  router.route("/OnSavebuttonAPI").post(function (req, res) {
    ...
    createTransferOrderItemsDetails(requestBody)
      .then((createResult) => { 
          ...
          let objTransferType = req.body.TransferOrder_Type == 1 ? "UAT Transfer Order WH To WH" : "UAT Transfer Order WH To Site";
          let mailData = {
              ...
              subjectEmail: objTransferType + ' - ' + createResult.InterfaceBatchNumber + " Pending for Approval"
          };
      ...
      .catch((createError) => {
        ...
        dataconn.errorlogger("createResult", "Save Transaction Successfully", createError);
        res
          .status(200)
          .json({
            Success: false,
  ...
  async function DeleteTransferDetails(reqTransferOrder_ID) {
    return new Promise(async (resolve, reject) => {

        const T_TransferOrderItems = datamodel.T_TransferOrderItems();

        T_TransferOrderItems.destroy({
            where: {
              TransferOrder_ID: reqTransferOrder_ID
            }
        }).then(() => {
            resolve();
        })
            .catch((err) => {
                dataconn.errorlogger('transaction', 'DeleteTransferDetails', err);
                reject();
            })
  ...
  router.route("/UpdateTransferDetails").post(function (req, res) {
    ...
    DeleteTransferDetails(requestBody.TransferOrder_ID).then(async (result) => {
      console.log('DeleteTransferDetails');
    ...
    UpdateTransferDetails(requestBody)
      .then(async (result) => {
        ...
        if (result != null) {
         ...
         let mailData = {
             ...
             toEmail: configuration.EmailIds.ForApproval.ToEmailIds,
             ccEmail : configuration.EmailIds.ForApproval.CcEmailIds,
             subjectEmail: objTransferType + ' - ' + masterdata[0].InterfaceBatchNumber + " Pending for Approval"
         };
        const emailTemplatePath = path.join(__dirname + '/../../Templates/TransferingOrder/Pending.ejs');
      ...
      .catch((error) => {
        ...
        dataconn.errorlogger("TransactionService", "UpdateTransferDetails", error);
  ...
  async function UpdateTransferDetails(request) {
    return new Promise(async (resolve, reject) => {
      ...
      connect.sequelize.transaction().then((trans) => {
        ...
        dataaccess.UpdateWithTransaction(TransferOrderMaster, values, param, trans)
          .then(
            function (result) {
              if (result != null) {
                console.log("Update TransferDetails............",reqBody.UpdateDetails);
                const UpdateDetails = datamodel.T_TransferOrderItems();
                var mapDetails = [];
                var promiseDetails = reqBody.UpdateDetails.map(function (mapitem) {
                  mapDetails.push({
                    TransferOrder_ID: reqBody.TransferOrder_ID,
                    INVENTORY: mapitem.INVENTORY,
                    ITEM_NUMBER: mapitem.ITEM_NUMBER,
                    ...
                    From_SUBINVENTORY: mapitem.From_SUBINVENTORY,
                    To_SUBINVENTORY: mapitem.To_SUBINVENTORY,
                    To_LOCATION: mapitem.LocationName,
                    Locator: mapitem.Locator,
                    Transfer_Qty: mapitem.Transfer_Qty
                ...
                Promise.all(promiseDetails).then(function () {
                  dataaccess.BulkCreateWithTransaction(UpdateDetails, mapDetails, trans)
                    .then((result) => {
                      console.log(result);
  ...
  router.route("/GetTransferOrderItemsDetails/:TransferOrder_ID").get(function (req, res) {
    const T_TransferOrderItems = datamodel.T_TransferOrderItems();

    console.log("req.params", req.params);
    var param = {
      where: {
        TransferOrder_ID: req.params.TransferOrder_ID
      },
  ...
  router.route("/GettransactionIdDetails/:TransferOrder_ID").get(function (req, res) {
    const M_TransferOrder = datamodel.M_TransferOrder();
    var param = {
  ...
  router.route("/TransferOrderItemsTable").post(function (req, res) {
    ...
    dataaccess.Create(T_TransferOrderItems, M_TransferOrder, values, param).then(
      function (result) {
        if (result != null) {
          res.status(200).json({
            Success: true,
  ...
  /////approve function is push into data base///////////////////////
    ...
    try {
      ...
      if (transOrderItemStatusData.length == result.items.length) {
       ...
       let mailData = {
           ...
           toEmail: configuration.EmailIds.ForApproval.ToEmailIds,
           ccEmail : configuration.EmailIds.ForApproval.CcEmailIds,
           subjectEmail: objTransferType + ' - '  + masterdata[0].InterfaceBatchNumber + " Approved"
       };
       ...
       console.log("end mail");
        ...
        res.status(200).json({Success: true,Message: "Transfer Order Approved successfully",Data: null,});
        return;
        }
      }
      res
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        transferOderItems.forEach(async (element) => {
          ...
          console.log("Seqeuence   99999999999", Seqeuence9, Seqeuence7);
          const T_TransferOrderItems = datamodel.T_TransferOrderItems();
          var values = {
            Seq7: Seqeuence7,
            Seq8: Seqeuence8,
  ...
  async function updateLineItemData(lineItemsData) {
    ...
    try {
      let M_TransferOrder = datamodel.M_TransferOrder();
      let T_TransferOrderItems = datamodel.T_TransferOrderItems();
      console.log("$$$$$$$$$$$$$ Updating Line Itemsss $$$$$$$$$$$$$$");
      ...
      console.log("promiseAr-err:Update Line Item Data", error);
      throw new Error(error)
    ...
    try {
      ...
      let promiseAr = itemsData.map(async e => {
        ...
        if (lineData.status == 200) {
          ...
          let requestBody = {
            OrganizationCode:lineData.data.items[0].SourceOrganizationCode.trim(),
            ItemNumber: lineData.data.items[0].ItemNumber,
            DemandSourceType: "Transfer order",
            DemandSourceHeaderNumber: lineData.data.items[0].TransferOrderHeaderNumber, // Number(Data.TransferOrderHeaderNumber),
            DemandSourceLineNumber:lineData.data.items[0].TransferOrderLineNumber, //1,//Number(Data.TransferOrderLineNumber),
      ...
      console.log("Promise-err:Reservation API", error);
      throw new Error(error)
    }
  }
  ...
  async function callSupplyLineApi(supplyOrderReferenceNumber) {
    ...
    // console.log("supplyOrderReferenceNumberrrrrrrrrrrrrrrr",supplyOrderReferenceNumber);
  ...
  router.route('/CancelTransferAPI').post(async (req, res) => {
    try {
      ...
      error="file not found its showing 404"
      console.log("error", error);
  ...
  async function TransferOrderCancelAPI(request) {
    return new Promise((resolve, reject) => {
      ...
      console.log("Transfer Cancel Done - ", data,requestBody.TransferOrder_ID)

      console.log("requestBody.transferOrderLines[0].TransferOrderHeaderId", + requestBody.transferOrderLines[0].TransferOrderHeaderId)//'https://fa-etcj-test-saasfaprod1.fa.ocs.oraclecloud.com/fscmRestApi/resources/11.13.18.05/transferOrders/' + requestBody.transferOrderLines[0].TransferOrderHeaderNumber,//
      axios({

        method: configuration.cancelOrderAPI.configData.method,
        url:  configuration.cancelOrderAPI.configData.url + requestBody.transferOrderLines[0].TransferOrderHeaderId,
        ...
        if (response.status == 200 || response.status == 201) {
          ...
          await dataaccess.Update(M_TransferOrder, values, param)
          dataconn.apiResponselogger('Transfer Cancel Order', requestBody.TransferOrder_ID, 0, error.response.status, error.response.data, 1);
        }
        ...
        if (error.response) {
          dataconn.apiResponselogger('Transfer Order Cancel API',requestBody.TransferOrder_ID, 0, error.response.status, error.response.statusText, 1);
        }
        reject();
      });
    });
